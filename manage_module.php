...
if(isset($_GET['id']))
	{
		$id = $_GET['id'];
		$delete_single_row = "DELETE FROM tblmodulename WHERE id='$id'";
...
if(isset($_POST['delete_selected']))
	{
		foreach($_POST['linkID'] as $chckvalue)
        	{
		    	$sql = "DELETE FROM tblmodulename WHERE id='$chckvalue'";
...
<div class="row" id="content">
    ...
    <div class="col-md-12">
        ...
        <form name='fullform' method='post' onSubmit="return confirmdelete()">
           ...
           <input type='hidden' name='pagename' value='users'>
     		<input type='button' name='cancel' class="btn btn-primary" value="Add New" onClick="window.location.replace('add_module.php?token=<?php echo $token ?>')"/>
       &nbsp;&nbsp;&nbsp;
   ...
   <?php
     ...
     $linkSQL = "select * from tblmodulename where 1=1 $where order by module_name";
   ...
   <?php } ?>
	  ...
	  <td><?php if($row["id"]!=1){?><a href="#" onClick="if(confirm('Do you really want to delete this record?')){ window.location.href='manage_module.php?id=<?php echo $row["id"]; ?>&type=del&token=<?php echo $token ?>' } " ><img src="images/drop.png" title="Delete" border="0" /></a> <?php } ?>    <?php if($row["id"]!=1){?> <a href="add_module.php?id=<?php echo $row["id"] ?>&token=<?php echo $token ?>"><img src='images/edit.png' title='Edit' border='0' /></a><?php } else {?> <a href="add_module.php?cid=<?php echo $row["id"] ?>&token=<?php echo $token ?>"><img src='images/edit.png' title='Edit' border='0' /></a> <?php } ?> &nbsp;&nbsp;<?php if($row["id"]!=1){?><input type='checkbox' name='linkID[]' value='<?php echo $row["id"]; ?>'><?php } ?> </td>
      </tr>
<?php }
		echo $pagerstring;
	  }
