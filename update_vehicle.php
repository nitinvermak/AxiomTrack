...
<div class="row" id="content">
   ...
   $linkSQL = "select * from tblvehile where 1=1 $where order by reg_no";
   ...
   <?php echo $_SESSION['sess_msg'];$_SESSION['sess_msg']='';?></td>
   <?php } ?>
   <tr>
   ...
   <a href='#' onClick="SetAllCheckBoxes('fullform','linkID[]',false)" style="color:#fff; font-size:11px;">Uncheck All </a> 
	  ...
	  if(mysql_num_rows($oRS)>0)
 	  ...
 	  ?>
      <tr <?php print $class?>>
      <td><?php echo stripslashes($row["reg_no"]);?></td>
	  <td><?php echo stripslashes($row["category_id"]);?></td>
	  <td><?php if($row["id"]!=1){?><a href="#" onclick="if(confirm('Do you really want to delete this record?')){ window.location.href='transusers_del.php?id=<?php echo $row["id"]; ?>&type=del&token=<?php echo $token ?>' } " ><img src="images/drop.png" title="Delete" border="0" /></a> <?php } ?>    <?php if($row["id"]!=1){?> <a href="update-vehicle-details.php?token=<?php echo $token ?>"><img src='images/edit.png' title='Edit' border='0' /></a><?php } else {?> <a href="change_password.php?cid=<?php echo $row["id"] ?>&token=<?php echo $token ?>"><img src='images/edit.png' title='Edit' border='0' /></a> <?php } ?> &nbsp;&nbsp;<?php if($row["id"]!=1){?><input type='checkbox' name='linkID[]' value='<?php echo $row["user_id"]; ?>'><?php } ?> </td>
	  </tr>
<?php }
		echo $pagerstring;
	  }
