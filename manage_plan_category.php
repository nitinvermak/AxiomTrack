...
if(isset($_GET['id']))
	{
		$id = $_GET['id'];
		$delete_single_row = "DELETE FROM tblplancategory WHERE id='$id'";
...
if(isset($_POST['delete_selected']))
	{
		foreach($_POST['linkID'] as $chckvalue)
        	{
		    	$sql = "DELETE FROM tblplancategory WHERE id='$chckvalue'";
				// Call User Activity Log function
...
<div class="row" id="content">
    ...
    <div class="col-md-12">
    	<div class="col-md-4 btn_grid">
     		<input type='button' name='cancel' class="btn btn-primary btn-sm" value="Add New" onClick="window.location.replace('plan_category.php?token=<?php echo $token ?>')"/>
       &nbsp;&nbsp;&nbsp;
    ...
    <div class="table-responsive">
      ...
      <?php
		...
		$linkSQL="";		
     	...
     	$linkSQL = "select * from tblplancategory where 1=1 $where order by category";
   		$pagerstring = Pages($linkSQL,PER_PAGE_ROWS,'manage_plan_category.php?',$token);
   ...
   <?php } ?>
	  ...
	  <td><a href="#" onClick="if(confirm('Do you really want to delete this record?')){ window.location.href='transusers_del.php?id=<?php echo $row["id"]; ?>&type=del&token=<?php echo $token ?>' }" ><img src="images/drop.png" title="Delete" border="0" /></a>     <a href="plan_category.php?id=<?php echo $row["id"] ?>&token=<?php echo $token ?>"><img src='images/edit.png' title='Edit' border='0' /></a> &nbsp;&nbsp;<input type='checkbox' name='linkID[]' value='<?php echo $row["user_id"]; ?>'></td>
      </tr>
<?php }
		echo $pagerstring;
	  }
