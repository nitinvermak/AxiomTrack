...
$(document).ready(function () {
	 console.log("HELLO")
          ...
          $(document).on("click","#export", function(){
                ...
                var outputFile = window.prompt("Please Enter the name your output file.") || 'DeviceStatus';
    ...
    <div class="col-md-12 table-responsive" id="activeVehicle">
     ...
     <td>
     	<select name="serviceAreaMgr" id="serviceAreaMgr"  class="form-control drop_down-sm">
				 ...
				 ?>
                 <option value="<?php echo $resultCountry['id']; ?>" 
				 <?php if(isset($result['service_area_manager']) && $resultCountry['id']==$result['service_area_manager']){ ?>selected<?php } ?>><?php echo stripslashes(ucfirst($resultCountry['First_Name']." " .$resultCountry['Last_Name'] )); ?>                 </option>
                 <?php } ?>
          </select>     </td>
      ...
      <tr>
        ...
        <td>
        	<select name="leadGenBy" id="leadGenBy"  class="form-control drop_down-sm">
                 ...
                 <option value="0">All</option>
                 <?php $Country=mysql_query("select * from tbluser where User_Category <> 1 Order by First_Name");
					   while($resultCountry=mysql_fetch_assoc($Country)){
				 ?>
                 <option value="<?php echo $resultCountry['id']; ?>" 
				 <?php if(isset($result['service_area_manager']) && $resultCountry['id']==$result['service_area_manager']){ ?>selected<?php } ?>><?php echo stripslashes(ucfirst($resultCountry['First_Name']." " .$resultCountry['Last_Name'] )); ?>                 </option>
