...
$sql="update tblmodulename set module_name='$datasource' where id=" .$_REQUEST['id'];
mysql_query($sql);
$_SESSION['sess_msg']='Module updated successfully';
header("location:manage_module.php?token=".$token);
exit();
}
else{
$queryArr=mysql_query("select * from tblmodulename where module_name ='$datasource'");
//$result=mysql_fetch_assoc($queryArr);
 if(mysql_num_rows($queryArr)<=0)
{
$query=mysql_query("insert into tblmodulename set  module_name='$datasource' ");
$_SESSION['sess_msg']='Module added successfully';
header("location:manage_module.php?token=".$token);
exit();
}
else
...
$queryArr=mysql_query("select * from tblmodulename where id =".$_REQUEST['id']);
$result=mysql_fetch_assoc($queryArr);
}
?>
<!DOCTYPE html>
...
<div class="row" id="content">
    ...
    <div class="col-md-12">
        ...
        <tr >
        <td>Add Module*</td>
        <td><input type="text" name="country" class="form-control text_box" value="<?php if(isset($result['id'])) echo $result['module_name'];?>" /></td>
        </tr>
        <tr>
        <td></td>
