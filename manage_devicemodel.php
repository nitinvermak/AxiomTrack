...
<div class="row" id="content">
    ...
    <input type='hidden' name='pagename' value='users'>
    	<div class="col-md-4 btn_grid">
     		<input type='button' name='cancel' class="btn btn-primary" value="Add New" onClick="window.location.replace('devicemodel.php?token=<?php echo $token ?>')"/>
       &nbsp;&nbsp;&nbsp;
   ...
   <?php
     ...
     $linkSQL = "select * from tbldevicemodel where 1=1 $where order by model_name";
     $pagerstring = Pages($linkSQL,PER_PAGE_ROWS,'manage_devicemodel.php?',$token);
   ...
   <?php } ?>
	  ...
	  <td><?php if($row["id"]!=1){?><a href="#" onClick="if(confirm('Do you really want to delete this record?')){ window.location.href='manage_devicemodel.php?id=<?php echo $row["device_id"]; ?>&type=del&token=<?php echo $token ?>' } " ><img src="images/drop.png" title="Delete" border="0" /></a> <?php } ?>    <?php if($row["id"]!=1){?> <a href="devicemodel.php?id=<?php echo $row["device_id"] ?>&token=<?php echo $token ?>"><img src='images/edit.png' title='Edit' border='0' /></a><?php } else {?> <a href="change_password.php?cid=<?php echo $row["id"] ?>&token=<?php echo $token ?>"><img src='images/edit.png' title='Edit' border='0' /></a> <?php } ?> &nbsp;&nbsp;<?php if($row["id"]!=1){?><input type='checkbox' name='linkID[]' value='<?php echo $row["device_id"]; ?>'><?php } ?></td>
      </tr>
<?php }
		echo $pagerstring;
	  }
