...
if ($sim_providerl == 0)
	{
	$linkSQL = "select * from tbl_sim_branch_assign where branch_confirmation_status= '1' and technician_assign_status='0' order by sim_id desc";
	}
	else
		{
		echo $branch_id;
		$linkSQL = "select * from tblsim as A, tbl_sim_branch_assign as B where A.id = B.sim_id and B.branch_id='{$sim_provider}' and A.company_id='{$sim_provider}'";
 		}
