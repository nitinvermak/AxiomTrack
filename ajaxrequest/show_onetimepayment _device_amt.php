...
error_reporting(0);
if($cust_id != ""){
		$sql = "SELECT A.Vehicle_id as vId, A.devicepaymentStatus as devicepaymentStatus, 
				B.vehicle_no as vNo
				FROM tbl_gps_vehicle_payment_master as A 
				INNER JOIN tbl_gps_vehicle_master as B
				ON A.Vehicle_id = B.id 
				WHERE A.devicepaymentStatus = 'N'
		...
		echo "<div class='col-md-12' style='margin:0px 0px 50px 0px; padding:0px;'><h3>Device Pending Amount</h3></div>";
        ...
        <?php
 	  	...
 	  	?>
				<tr>
					...
					<td><small><?php if($row['devicepaymentStatus'] == 'N'){ echo 'No'; } ?></small></td>
					<td><input type='checkbox' name='linkID[]' id="linkID" value='<?= $row["vId"]; ?>'></td>
				</tr>
<?php 
				}
				echo "</table>";
				echo "<input type='submit' name='upadteDeviceAmt' value='Submit' onclick='return val();' id='submit' class='btn btn-primary btn-sm'>";
				echo "</form>";
			}
			else{
