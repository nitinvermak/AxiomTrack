...
//echo $linkSQL;
	...
	$linkSQL = "SELECT A.ticket_id as tId, C.Company_Name as CompanyName, A.description as description,
				C.Mobile as mobile, F.category as product, A.vehicleId as vehicleId, A.repairReason as reason,
				E.rqsttype as requestType, A.createddate as cDate, C.id as callingDataId,
				B.assign_by as assignBy, A.appointment_date as appiontmentDate 
				FROM tblticket as A 
				INNER JOIN tbl_ticket_assign_branch as B 
				ON A.ticket_id = B.ticket_id 
...
?>		
	...
	<?php
 	...
 	?>
				 ...
				 <input type="hidden" name="callingDataId" value="<?= $row["callingDataId"]; ?>">
				 </small></td>
                 ...
                 <input type="hidden" name="vehicleNo" id="vehicleNo" value="<?php echo getVehicleNumber(stripslashes($row["vehicleId"])); ?>" />
                 </td>
...
?> 
                          ...
                          <table>
                          <tr>
                          <td colspan="3"><input type="submit" name="submit_tech" class="btn btn-primary btn-sm" onClick="return val();" value="Submit" id="submit" /> </td>
                          <td></td>
                          </tr>
                          </table>
                   	    </form>   
