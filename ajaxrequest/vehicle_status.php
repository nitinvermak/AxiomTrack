...
if ($cust_id == '')
	...
	$linkSQL = "Please Provide Search Criteria";
			...
			{
				$linkSQL = "SELECT A.cust_id, A.customer_type, A.np_device_amt, B.id as vehicleId, 
...
?>	
	...
	<?php
 	...
 	?>
			...
			{
			?>
            <input type="button" name="Inactive" id="Inactive" class="btn btn-success btn-sm" value="Active" onClick="getInactive(<?php echo stripslashes($row['vehicleId']); ?>);">
			<?php 
			}
			else
			{
			?>
			<input type="button" name="Active" id="Active" class="btn btn-danger btn-sm" value="Inactive" onClick="getActive(<?php echo stripslashes($row['vehicleId']); ?>);">
            <?php
			}
			?>
            </td>
