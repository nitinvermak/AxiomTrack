...
if(isset($_POST['submit']))
	{
		...
		$calling_status = '1';
		$update_records = "Update tblcallingdata set First_Name='$first_name', Last_Name='$last_name', 
						   Company_Name='$company', Phone='$phone', Mobile='$mobile', email='$email', 
						   Address='$Address', Area='$area', City='$city', State='$state', 
						   Country='$country', Pin_code='$pincode', calling_status='$calling_status' 
		...
		$device_rent = mysql_real_escape_string($_POST['device_rent']);
		$installation_charges = mysql_real_escape_string($_POST['installation_charges']);
		$c_device_rent = mysql_real_escape_string($_POST['c_device_rent']);
		$c_installation_charges = mysql_real_escape_string($_POST['c_installation_charges']);
		$follow_date = mysql_real_escape_string($_POST['follow_date']);
		$reason = mysql_real_escape_string($_POST['reason']);
		$remarks = mysql_real_escape_string($_POST['remarks']);
		if($calling_status == "1")
		{
			$update_calling_status = "update tbl_telecalling_status set calling_date='$callingdate', 
									  device_model_id='$model', calling_status='$calling_status', 
									  no_of_vehicles='$no_of_vehicles', np_device_amt='$p_device_amt', 
									  np_device_rent='$p_device_rent', rent_payment_mode='$payment_type', 
									  r_device_rent='$device_rent', r_installation_charge='$installation_charges', 			     	    							  	  c_device_rent='$c_device_rent', c_installation_charge='$c_installation_charges', 
									  follow_up_date='$follow_date', not_interested_resason='$reason', 
									  remark_not_interested='$remarks' 
									  where callingdata_id='$update_id'";
			// Call User Activity Log function
			UserActivityLog($_SESSION['user_id'], $_SERVER['REMOTE_ADDR'], $_SERVER['PHP_SELF'], $update_calling_status);
			// End Activity Log Function
			header("location:follow_up_telecalling.php?token=".$token);
			$query = mysql_query($update_calling_status);
		}
	}
if(isset($_REQUEST['id']) && $_REQUEST['id'])
	{
		$queryArr=mysql_query("SELECT * FROM tblcallingdata as A,
							   tbl_telecalling_status as B 
							   WHERE A.id = B.callingdata_id 
							   and B.callingdata_id =".$_REQUEST['id']);
	...
	if(isset($_POST['submit1']))
		{
			...
			$confirm_client = "insert into tbl_customer_master set 	callingdata_id='$update_id',   												 							   calling_product='$calling_products', device_model_id='$model', 
							   ...
							   rent_payment_mode='$payment_type', r_device_rent='$device_rent', 																																														 							   r_installation_charge='$installation_charges', c_device_rent='$c_device_rent', 		 		      		  							   c_installation_charge='$c_installation_charges',confirmation_date=Now()";
			// Call User Activity Log function
			UserActivityLog($_SESSION['user_id'], $_SERVER['REMOTE_ADDR'], $_SERVER['PHP_SELF'], $confirm_client);
			// End Activity Log Function
			$result = mysql_query($confirm_client);
			$change_status = "UPDATE tblcallingdata SET status ='1' where id = '$update_id'";
			// Call User Activity Log function
			UserActivityLog($_SESSION['user_id'], $_SERVER['REMOTE_ADDR'], $_SERVER['PHP_SELF'], $result);
			// End Activity Log Function
...
<div class="row" id="content">
    ...
    <div class="col-md-12 table table-responsive" id="contactform"> <!--open of the single form-->
         ...
         <td><span style=" padding-right:5px">
           <input name="last_name" id="last_name" class="form-control text_box" value="<?php if(isset($result['id'])) echo $result['Last_Name']; ?>"  type="text" />
         </span></td>
        </tr>
        <tr>
        <td>Company Name*</td>
...
WHERE A.id = B.callingdata_id and B.callingdata_id ='$id'");
    ...
    <table class="table">
    <tr>
    <td class="col-sm-2"><strong>New Purchase</strong></td>
    <td valign="top">&nbsp;</td>
    <td></td>
    <td>&nbsp;</td>
    </tr>
    ...
    <td valign="top"><input type="text" name="no_of_vehicles"  class="form-control text_box" id="no_of_vehicles" value="<?php if(isset($result['id'])) echo $result['no_of_vehicles']; ?>" /></td>
    <td>Device Amt</td>
    <td><select name="p_device_amt" id="p_device_amt" class="form-control drop_down">
        <option value="">Select</option>
        <?php $Country=mysql_query("select * from tblplan where plan_category = 4 and plan_description='Device Rate'");
			  while($resultCountry=mysql_fetch_assoc($Country)){
        ?>
    ...
    <td>Device  Rent*</td>
    <td valign="top">
    <select name="p_device_rent" id="p_device_rent" class="form-control drop_down">
    <option value="">Select</option>
    ...
    <tr>
    <td>Device Rent</td>
    <td><select name="device_rent" id="device_rent" class="form-control drop_down">
          <option value="">Select</option>
              <?php $Country=mysql_query("select * from tblplan where plan_category = 4 and plan_description='Device_On_Rent'");
					while($resultCountry=mysql_fetch_assoc($Country)){
			   ?>
          <option value="<?php echo $resultCountry['id']; ?>" <?php if(isset($result['r_device_rent']) && $resultCountry['id']==$result['r_device_rent']){ ?>selected<?php } ?>><?php echo stripslashes(ucfirst($resultCountry['plan_rate'])); ?></option>
          <?php } ?>
        </select></td>
    <td>Installation Charges</td>
    <td><select name="installation_charges" id="installation_charges" class="form-control drop_down">
           <option value="">Select</option>
    ...
    <tr>
    <td valign="top"><strong>Customer Device</strong></td>
    <td valign="top">&nbsp;</td>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
    </tr>
    <tr>
    <td valign="top">Device Rent</td>
    <td valign="top"><select name="c_device_rent" id="c_device_rent" class="form-control drop_down">
    			      <option value="">Select</option>
            <?php $Country=mysql_query("select * from tblplan where plan_category = 4 and plan_description='Rental'");
				  while($resultCountry=mysql_fetch_assoc($Country)){
			?>
              <option value="<?php echo $resultCountry['id']; ?>" <?php if(isset($result['c_device_rent']) && $resultCountry['id']==$result['c_device_rent']){ ?>selected<?php } ?>><?php echo stripslashes(ucfirst($resultCountry['plan_rate'])); ?></option>
            <?php } ?>
          </select></td>
    <td>Installation Charges</td>
    <td><select name="c_installation_charges" id="c_installation_charges" class="form-control drop_down">
          <option value="">Select</option>
    	   <?php $Country=mysql_query("select * from tblplan where plan_category = 4 and plan_description='Installtion_Charges'");
				 while($resultCountry=mysql_fetch_assoc($Country)){
			?>
          <option value="<?php echo $resultCountry['id']; ?>" <?php if(isset($result['c_installation_charge']) && $resultCountry['id']==$result['c_installation_charge']){ ?>selected<?php } ?>><?php echo stripslashes(ucfirst($resultCountry['plan_rate'])); ?></option>
          <?php } ?>
       </select></td>
    </tr>
   <tr>
   ...
   <tr> </tr>
    ...
    <input type='button' name='cancel' class="btn btn-primary" value="Back" 
	 onclick="window.location='follow_up_telecalling.php?token=<?php echo $token ?>'"/>
   	<input type='submit' name='submit1' id="confirm" class="btn btn-primary" value="Confirm Client" />
    <input type='button' name='gen_ticket' class="btn btn-primary" value="Generate Ticket" onClick="showModalPopUp()"/>
    </td>
    <td valign="top">&nbsp;</td>
    </tr>
    </table>
